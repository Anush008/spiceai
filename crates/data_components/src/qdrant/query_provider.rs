...
/// Canonical name is `search::SEARCH_SCORE_COLUMN_NAME`; duplicated here because
/// `search` already depends on this crate, so the dependency cannot be reversed.
...
fn build_embedding_array(
    ...
    // `NullBuffer` carries validity bits in this Arrow fork: `true` marks a
    // valid (non-NULL) slot.
...
impl ExecutionPlan for QdrantListExec {
    ...
    fn execute(
        ...
        // A manual `unfold` stream rather than `async_stream::stream!`, which
        // obscures backtraces and breaks rust-analyzer jump-to-definition.
        struct ScrollState {
            client: Arc<dyn QdrantStore>,
            collection: String,
            schema: SchemaRef,
...
mod tests {
    ...
    #[test]
    fn embedding_array_marks_missing_vectors_null_and_present_vectors_valid() {
        let results = vec![
    ...
    #[test]
    fn embedding_array_errors_when_stored_vector_has_wrong_dims() {
        let results = vec![result_with_vector(Some(vec![1.0]), true)];
        let err = build_embedding_array(&results, "embedding", 2).expect_err("dims mismatch");
        assert!(err.to_string().contains("vector length 1, expected 2"));
    }

    #[test]
    fn empty_results_produce_empty_embedding_array() {
        let array = build_embedding_array(&[], "embedding", 4).expect("array");
    ...
    fn batch_scores(semantics: QdrantScoreSemantics) -> f64 {
        let results = vec![SearchResult {
            score: 0.5,
            ..result_with_vector(None, false)
        }];
        let batch = search_results_to_batch(&results, &score_schema(), "embedding", 2, semantics)
            .expect("batch");
        batch
            .column(0)
            .as_any()
    ...
    fn dot_scores_pass_through_unchanged() {
        assert_eq!(batch_scores(QdrantScoreSemantics::Similarity), 0.5);
    }
    ...
    fn cosine_scores_normalize_to_match_one_minus_cosine_distance() {
        // cosine_distance UDFs return (1 - similarity) / 2 ∈ [0, 1], so the
        // engine path must emit `1 - cosine_distance`, not raw similarity.
        assert_eq!(batch_scores(QdrantScoreSemantics::NormalizedCosine), 0.75);
    }
    ...
    fn distance_metrics_negate_so_higher_is_better() {
        assert_eq!(batch_scores(QdrantScoreSemantics::NegatedDistance), -0.5);
    }
}
