...
async fn get_store_params(
    ...
    // Rejected before typed parsing so any value (not just `true`) fails loudly
    // instead of being silently dropped by unknown-parameter handling.
    if params.iter().any(|(k, _)| k == "qdrant_spill_writes") {
        return Err(
            "`qdrant_spill_writes` is not supported for the Qdrant vector engine. \
            Remove the parameter. See: https://spiceai.org/docs/components/vector-databases"
                .into(),
        );
    }
...
mod tests {
    ...
    #[test]
    fn partition_key_derives_from_column_expr() {
    ...
    #[test]
    fn partition_key_none_without_exprs() {
    ...
    #[test]
    fn partition_key_rejects_multiple_exprs() {
