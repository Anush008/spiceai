...
async fn wrap_table_as_index_qdrant(
    ...
    // Every column's vector index is collected here and joined onto a single
    // `VectorScanTableProvider` after the loop, rather than each column wrapping the
    // previous column's layer in its own nested provider.
    let mut vector_indexes: Vec<Arc<dyn VectorIndex>> = Vec::new();
    ...
    for (column, config) in embedding_columns {
        ...
        // Mirror the Elasticsearch engine: the query and list plans expose the full
        // (augmented, when chunked) primary key plus the metadata columns, so the
        // in-memory warm index can fall back onto Qdrant for both chunked and
        // non-chunked columns.
