...
impl VectorSearchTableFunc {
    ...
    fn index_based_vector_table(
        ...
        // Chunked search indexes (used by the S3 Vectors, Elasticsearch, and Qdrant
        // engines when chunking is enabled) are discovered once here to avoid
        // registering the same `ChunkedSearchIndex` twice when multiple engine
